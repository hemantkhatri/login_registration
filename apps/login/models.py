...
class UserManager(models.Manager):
	...
	def register(self, first_name, last_name, email, password, confirm_password ):
		...
		if (not emailRegex.match(email)):
			...
			email_register = User.userMgr.filter(email=email)
			print email_register,
			if (email_register):
				if (email==email_register[0].email):
					errors.append("Email exists in our system ")
	...
	def login(self, email, password):
		...
		if user:
			...
			errors.append("No email found in our system, please register.")
			return (False, errors)
